...
(function () {
  ...
  this.onPluginLoad = function (context) {
    ...
    let dynamicCulture=customHeader.getSessionStorage('homepageLang') || context.culture
    let availableCultures=['en','ru','en-US']
    if(availableCultures.indexOf(dynamicCulture)==-1)
      dynamicCulture='en'
    customHeader.dynamicallyLoadScript(dynamicCulture);
...
customHeader.dynamicallyLoadScript = function(selectedValue=false){
    if(!selectedValue){
    ...
    customHeader.setSessionStorage('homepageLang',selectedValue)
    console.log("url--------------", url);
