...
font-size: 1.125rem;
  ...
  this.buildHTML = function () {
    ...
    customHomepage.transactionName = Transaction;
...
style="background-image: url(${value.img});">
  ...
  this.handleAction = function (item, nameOfMainJs) {
    ...
    switch (item.action) {
      ...
      case 'createNewTransaction':
        return `customHomepage.createNewOrder('${item.catalog}','${item.transaction}','${deepLink}',true)`;
      case 'zendesk':
  ...
  customHomepage.setUUIDandNav = function (in_catalog = null, in_transactionName = null, deepLink = null) {
    ...
    if (uuid && uuid !== "undefined") {
      ...
      customHomepage.createNewOrder(in_catalog, in_transactionName, deepLink, false);
    }
  };
  ...
  customHomepage.createNewOrder = function (inCatalog = null, in_transactionName = null, deepLink = null, skipSessionSaving) {
    customHomepage.catalogName = Catalog
    let catalogUUID = !inCatalog ? this.catalogs.find((el) => el.ExternalID === customHomepage.catalogName).UUID : this.catalogs.find((el) => el.ExternalID === inCatalog).UUID
    var bridgeObject = {
      references: {
        account: {
